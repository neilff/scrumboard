...
exports.remove_from_room = function(client, room, callback) {
    sid_rooms[client.id].remove(room);

    if (!rooms.hasOwnProperty(room) || !rooms[room].has(client)) {
      console.log('Attempted to remove ', + client.id + ' from room :: ', room, ' but failed');
    ...
    if (callback) {
      callback(exports.roomClients(room));
    }
}

// Return list of clients in the given room.
exports.roomClients = function(room) {
...
exports.on_leave_room = function(client, room, msg) {
  exports.remove_from_room(client, room, function(clients) {
    clients.map(user => {
      user.json.send(msg);
    });
  });
